...
export abstract class  Weapon extends Item {
    ...
    use() : string
    {
        if(this.getDurability() <= 0) return `You can't use the ${this.name} , it is broken.`;
        this.durabilityModifier -= this.MODIFIER_CHANGE_RATE;
        if(this.getDurability() <= 0) return `You use the ${this.name} , dealing ${this.getDamage().toFixed(2)} points of damage . The hammer breaks.`;
